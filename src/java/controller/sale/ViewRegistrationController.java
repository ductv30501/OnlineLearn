 ...
 * @author Admin
 */
@WebServlet(name = "ViewRegistrationController", urlPatterns = {"/authen/sale/registration/view"})
public class ViewRegistrationController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String isername = request.getParameter("id");
        int packageID = Integer.parseInt(request.getParameter("packageID"));
        // Step 2: Get user package by its id 
        UserPackageDBContext userPackage = new UserPackageDBContext();
        UserPackage userPack = userPackage.getUserPackageByUsername(isername,packageID);
        UserDBContext uDB = new UserDBContext();
        User userSale = uDB.getUser(userPack.getLastUpdateBy());
        User userlearner = uDB.getUser(isername);
        ...
        if (userPack.isStatus()) {
            userPackageDetail += "đã duyệt" + "|";
        } else {
            userPackageDetail += "chưa duyệt" + "|";
        }
        userPackageDetail += packageID +"|";
        userPackageDetail += userSale.getName() + "|";
        userPackageDetail += userlearner.getName() +"|";
        userPackageDetail += userlearner.getPhone() +"|";
        if (userlearner.isGender()) {
            userPackageDetail += "Nam" + "|";
        } else {
            userPackageDetail += "Nữ" + "|";
        }
